...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            ...
            audit_log(user.id, 'LOGIN', 'User', user.id, f'User {username} logged in')
            return redirect(url_for('dashboard'))
        else:
...
def logout():
    ...
    logout_user()
    flash(f'Sesión cerrada correctamente', 'info')
    return redirect(url_for('login'))
...
def new_asset():
    ...
    if request.method == 'POST':
        try:
            ...
            audit_log(current_user.id, 'CREATE', 'Asset', asset.id, f'Asset {barcode} created')
...
def edit_asset(asset_id):
    ...
    if request.method == 'POST':
        try:
            ...
            audit_log(current_user.id, 'UPDATE', 'Asset', asset.id, f'Asset {asset.barcode} updated')
...
def delete_asset(asset_id):
    ...
    if not current_user.is_admin():
        flash('No tiene permisos para eliminar activos', 'danger')
        return redirect(url_for('list_assets'))
    ...
    try:
        ...
        audit_log(current_user.id, 'DELETE', 'Asset', asset_id, f'Asset {barcode} deleted')
...
def transfer_asset(asset_id):
    ...
    if request.method == 'POST':
        try:
            ...
            audit_log(current_user.id, 'TRANSFER', 'Asset', asset_id, f'Asset {asset.barcode} transferred from location {transfer.from_location_id} to {transfer.to_location_id}')
...
def new_maintenance(asset_id):
    ...
    if request.method == 'POST':
        try:
            ...
            audit_log(current_user.id, 'MAINTENANCE', 'MaintenanceRecord', maintenance.id, f'Maintenance recorded for asset {asset.barcode}')
