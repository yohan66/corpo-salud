...
class User(UserMixin, db.Model):
    """User model for authentication with role hierarchy"""
    __tablename__ = 'users'
    ...
    # ============================================================
    # MÉTODOS DE VERIFICACIÓN DE ROLES - CONTROL TOTAL DEL SISTEMA
    # ============================================================

    def is_user(self):
        """Verifica si es usuario básico"""
        return self.role == 'user'

    def is_admin(self):
        """Verifica si es administrador (incluye superuser)"""
        return self.role in ['admin', 'superuser']

    def is_superuser(self):
        """Verifica si tiene control TOTAL del sistema"""
        return self.role == 'superuser'

    def has_full_control(self):
        """Método unificado para permisos absolutos"""
        return self.role == 'superuser' and self.is_active

    def can_manage_users(self):
        """Solo superuser puede gestionar usuarios"""
        return self.is_superuser()

    def can_delete_assets(self):
        """Admin y superuser pueden eliminar activos"""
        return self.is_admin()

    def can_edit_all(self):
        """Admin y superuser pueden editar todo"""
        return self.is_admin()

    def can_transfer_assets(self):
        """Todos los roles activos pueden transferir"""
...
def init_db(app):
    ...
    with app.app_context():
        ...
        # Create default SUPERUSER with FULL CONTROL
        if not User.query.filter_by(username='admin').first():
            ...
            admin = User(
                ...
                full_name='Administrador del Sistema - Control Total',
                role='superuser',
                is_active=True
            )
            db.session.add(admin)
            print('Superuser creado: admin / Admin2024!')
